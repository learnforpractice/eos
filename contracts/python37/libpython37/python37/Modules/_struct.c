...
static PyObject *
unpackiter_len(unpackiterobject *self, void *Py_UNUSED(ignored))
{
    Py_ssize_t len;
    if (self->so == NULL)
        len = 0;
...
static PyObject *py_pack_uint32(PyObject *self, PyObject *args)
{
    PyObject *o;
    uint64_t val;
    char result[8];
    ...
    val  = PyLong_AsUnsignedLongLong(o);
    if (val <= 127) {
    ...
    return PyBytes_FromStringAndSize(result, i+1);
}

static PyObject *py_unpack_uint32(PyObject *self, PyObject *args)
{
    PyObject *o;
    uint64_t val;
    char result[8];
    ...
    o = PyTuple_GetItem(args, 0);
    PyBytes_AsStringAndSize(o, &data, &size);
    uint64_t v = 0; uint8_t b = 0; uint8_t by = 0;
    int i;
    for (i=0;i<size;i++) {
        b = (uint8_t)data[i];
    ...
    o = PyTuple_New(2);
    PyTuple_SetItem(o, 0, PyLong_FromUnsignedLongLong(v));
    PyTuple_SetItem(o, 1, PyLong_FromLong(i+1));
    return o;
}
...
static struct PyMethodDef module_functions[] = {
    ...
    {"pack_uint32",             (PyCFunction)py_pack_uint32, METH_VARARGS, NULL},
    {"unpack_uint32",           (PyCFunction)py_unpack_uint32, METH_VARARGS, NULL},
    UNPACK_METHODDEF
