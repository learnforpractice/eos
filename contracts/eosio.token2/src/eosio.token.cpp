...
extern "C" { \
   __attribute__ ((visibility ("default"))) void apply( uint64_t receiver, uint64_t code, uint64_t action ) {
      if( code == receiver ) {
         switch( action ) {
            EOSIO_DISPATCH_HELPER( eosio::token, (create)(issue)(transfer)(open)(retire) )
         }
         /* does not allow destructor of thiscontract to run: eosio_exit(0); */
      }
   }
