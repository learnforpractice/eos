...
multi_index::multi_index(uint64_t code, uint64_t scope, uint64_t table, vector<index_type>& indexes): 
code(code),
scope(scope),
table(table)
{
...
void multi_index::store(uint64_t primary_key, void *data, uint32_t data_size, vector<vector<char>>& secondary_values, uint64_t payer) {
...
void multi_index::modify(int itr, uint64_t primary_key, void *data, uint32_t data_size, vector<vector<char>>& secondary_values, uint64_t payer) {
    check(secondary_values.size() == secondary_indexes.size(), "bad secondary value count");
    uint32_t i = 0;
...
int multi_index::find_secondary(int secondary_index, void *key, uint32_t key_size) {
...
int multi_index::lowerbound_secondary(int secondary_index, uint64_t code, uint64_t scope, uint64_t table, void* secondary, uint32_t secondary_size, uint64_t primary_key) {
    auto idx = secondary_indexes[secondary_index];
    uint64_t idx_table = get_secondary_idx_table(secondary_index);
...
int multi_index::upperbound_secondary(int secondary_index, uint64_t code, uint64_t scope, uint64_t table, void* secondary, uint32_t secondary_size, uint64_t primary_key) {
    auto idx = secondary_indexes[secondary_index];
    uint64_t idx_table = get_secondary_idx_table(secondary_index);
    return idx->db_idx_upperbound(code, scope, idx_table, secondary, secondary_size, primary_key);
}

bool multi_index::get_by_secondary_key(int secondary_index, void *key, uint32_t key_size, vector<char>& value) {
    check(secondary_index<secondary_indexes.size(), "bad secondary index");
    auto idx = secondary_indexes[secondary_index];
    uint64_t idx_table = get_secondary_idx_table(secondary_index);
    uint64_t primary_key;
