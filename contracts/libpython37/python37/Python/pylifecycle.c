                        ...
                        const _PyCoreConfig *core_config)
{
...
#if !defined(MS_WINDOWS) || !defined(__WASM)
    ...
    *interp_p = interp;
    ...
    Py_DECREF(pstderr);
#ifdef __WASM
    PyObject *pstdout = PyFile_NewStdPrinter(fileno(stdout));
    if (pstdout == NULL)
        return _Py_INIT_ERR("can't set preliminary stdout");
    _PySys_SetObjectId(&PyId_stdout, pstdout);
    PySys_SetObject("__stdout__", pstdout);
    Py_DECREF(pstdout);
#endif
...
{
#ifdef __WASM
    prints("++++Py_FatalError:");prints(msg);prints("\n");
#else
