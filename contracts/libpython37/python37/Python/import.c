...
{
    static struct _frozen _frozen_module;
    if (name == NULL)
        return NULL;
    ...
    _frozen_module.size = code_size;
    _frozen_module.code = frozen_memory;
    _frozen_module.name = module_name;
...
PyObject *
PyImport_ImportModuleObject(PyObject *name, const char *code, size_t size)
{
    const struct _frozen *p;
    PyObject *co, *m, *d;
    int ispackage;
    ...
    if (ispackage)
        size = -size;
    co = PyMarshal_ReadObjectFromString((const char *)code, size);
    if (co == NULL)
        return NULL;
    if (!PyCode_Check(co)) {
        PyErr_Format(PyExc_TypeError,
