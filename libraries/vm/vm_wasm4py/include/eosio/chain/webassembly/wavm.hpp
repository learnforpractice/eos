...
namespace eosio { namespace chain {
   ...
   void wavm_load_data_to_writable_memory(U32 ptr, size_t length);
}}
#endif
...
inline array_ptr<T> array_ptr_impl (running_instance_context& ctx, U32 ptr, size_t length)
{
   ...
   if (ptr >= mem_total || length > (mem_total - ptr) / sizeof(T))
      Runtime::causeException(Exception::Cause::accessViolation);
#ifdef WASM_FOR_PYTHON
   wavm_load_data_to_writable_memory(ptr, length);
#endif
...
template<typename T>
inline memory_addr<T> array_ptr2_impl (running_instance_context& ctx, U32 ptr, size_t length)
{
   MemoryInstance* mem = ctx.memory;
   if (!mem)
      Runtime::causeException(Exception::Cause::accessViolation);
...
//   T* ret_ptr = (T*)(getMemoryBaseAddress(mem) + ptr);
   ...
   return memory_addr<T>((T*)(getMemoryBaseAddress(mem) + ptr));
}
...
#ifdef WASM_FOR_PYTHON
      wavm_load_data_to_writable_memory(ptr++, 1);
#endif
      if(*p++ == '\0') {
...
template<typename Ret, typename... Inputs, typename ...Translated>
struct intrinsic_invoker_impl<Ret, std::tuple<memory_addr<char>, size_t, Inputs...>, std::tuple<Translated...>> {
   using next_step = intrinsic_invoker_impl<Ret, std::tuple<Inputs...>, std::tuple<Translated..., I32, I32>>;
   using then_type = Ret(*)(running_instance_context&, memory_addr<char>, size_t, Inputs..., Translated...);

   template<then_type Then>
   static Ret translate_one(running_instance_context& ctx, Inputs... rest, Translated... translated, I32 ptr_t, I32 offset) {
      ptr_t += offset;
      return Then(ctx, array_ptr2_impl<char>(ctx, (U32)ptr_t, 8), 8, rest..., translated...);
   };
...
#ifdef WASM_FOR_PYTHON
      wavm_load_data_to_writable_memory(ptr, sizeof(T));
#endif
      if ( reinterpret_cast<uintptr_t>(&base) % alignof(T) != 0 ) {
         if(get_chain_api()->contracts_console())
