...
#include "python_wavm.hpp"
#include "python_wabt.hpp"
...
namespace eosio { namespace chain {
   ...
   struct wasm_interface_impl {
      ...
      {
         return get_instantiated_module(code_id, code.data(), code.size());
      }

      std::unique_ptr<wasm_instantiated_module_interface>& get_instantiated_module( const digest_type& code_id, const char *data, size_t size)
      {
      ...
      void run_module( const char* data, size_t size )
      {
         IR::Module module;
         try {
            Serialization::MemoryInputStream stream((const U8*)data, size);
         ...
         std::unique_ptr<wasm_instantiated_module_interface> instance = runtime_interface->instantiate_module((const char*)bytes.data(), bytes.size(), parse_initial_memory(module));
         instance->call(N(apply), 0, 0, 0);
      }

      std::unique_ptr<wasm_instantiated_module_interface> load_python_module( const char* data, size_t size )
      {
         IR::Module module;
         try {
            Serialization::MemoryInputStream stream((const U8*)data, size);
         ...
         return runtime_interface->instantiate_module((const char*)bytes.data(), bytes.size(), parse_initial_memory(module));
      }
      ...
      void load_module( const digest_type& code_id, const char *data, int size) {
         if (loading_queue.contains(code_id)) {
            return;
         }
         vector<uint8_t> _code(data, data+size);
         struct code_info info = {code_id, _code};
         loading_queue.push(info);
      }

      void _load_module( const digest_type& code_id, const vector<uint8_t>& code)
      {
         map<digest_type, std::unique_ptr<wasm_instantiated_module_interface>>::iterator it;
         {
            std::lock_guard<std::mutex> guard(cache_mutex);
      ...
      bool load_module_async() {
         ...
         _load_module(info.code_id, info.code);
