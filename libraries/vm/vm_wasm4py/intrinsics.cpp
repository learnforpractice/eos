...
class softfloat_api : public context_aware_api {
   public:
      ...
      int _eosio_i32_load(memory_addr<char> ptr, size_t size) {
         return ((uint32_t*)ptr.value)[0];
      }

      int _eosio_i32_load8_s(memory_addr<char> ptr, size_t size) {
         return static_cast<int32_t>(((int8_t*)ptr.value)[0]);
      }

      int _eosio_i32_load8_u(memory_addr<char> ptr, size_t size) {
         return static_cast<uint32_t>(((uint8_t*)ptr.value)[0]);
      }

      int _eosio_i32_load16_s(memory_addr<char> ptr, size_t size) {
         return static_cast<int32_t>(((int16_t*)ptr.value)[0]);
      }

      int _eosio_i32_load16_u(memory_addr<char> ptr, size_t size) {
         return static_cast<uint32_t>(((uint16_t*)ptr.value)[0]);
      }
//                  i64_load
      int64_t _eosio_i64_load(memory_addr<char> ptr, size_t size) {
         return ((int64_t*)ptr.value)[0];
      }

      int64_t _eosio_i64_load8_s(memory_addr<char> ptr, size_t size) {
         return static_cast<int64_t>(((int8_t*)ptr.value)[0]);
      }

      int64_t _eosio_i64_load8_u(memory_addr<char> ptr, size_t size) {
         return static_cast<uint64_t>(((uint8_t*)ptr.value)[0]);
      }

      int64_t _eosio_i64_load16_s(memory_addr<char> ptr, size_t size) {
         return static_cast<int64_t>(((int16_t*)ptr.value)[0]);
      }

      int64_t _eosio_i64_load16_u(memory_addr<char> ptr, size_t size) {
         return static_cast<uint64_t>(((uint16_t*)ptr.value)[0]);
      }

      int64_t _eosio_i64_load32_s(memory_addr<char> ptr, size_t size) {
         return static_cast<int64_t>(((int32_t*)ptr.value)[0]);
      }

      int64_t _eosio_i64_load32_u(memory_addr<char> ptr, size_t size) {
         return static_cast<uint64_t>(((uint32_t*)ptr.value)[0]);
      }
...
class console_api : public context_aware_api {
   public:
      ...
      void _prints(null_terminated_ptr str) {
//         if ( !ignore ) {
         printf("prints: %s\n", (const char*)str);
         return;
         if ( true ) {
...
//python_wavm.cpp
void wavm_check_bounds(int start, int offset);
...
#else
         if((std::abs((ptrdiff_t)dest.value - (ptrdiff_t)src.value)) < length) {
               vmdlog("dest: %d, src: %d, length: %d\n", dest.value, src.value, length);
               vmdlog("memcpy can only accept non-aliasing pointers\n");
         }
#endif
...
#define __REGISTER_INTRINSIC_EXPLICIT(CLS, MOD, METHOD, WASM_SIG, NAME, SIG)\
   _REGISTER_WAVM_INTRINSIC(CLS, MOD, METHOD, WASM_SIG, BOOST_PP_STRINGIZE(f64-rem), SIG)\
   _REGISTER_WABT_INTRINSIC(CLS, MOD, METHOD, WASM_SIG, BOOST_PP_STRINGIZE(f64-rem), SIG)

#define __REGISTER_INTRINSIC4(CLS, MOD, METHOD, WASM_SIG, NAME, SIG)\
   __REGISTER_INTRINSIC_EXPLICIT(CLS, MOD, METHOD, WASM_SIG, NAME, SIG )

#define __REGISTER_INTRINSIC3(CLS, MOD, METHOD, WASM_SIG, NAME)\
   __REGISTER_INTRINSIC_EXPLICIT(CLS, MOD, METHOD, WASM_SIG, NAME, decltype(&CLS::METHOD) )


#define __REGISTER_INTRINSIC2(CLS, MOD, METHOD, WASM_SIG)\
   __REGISTER_INTRINSIC_EXPLICIT(CLS, MOD, METHOD, WASM_SIG, BOOST_PP_STRINGIZE(METHOD), decltype(&CLS::METHOD) )


#define __REGISTER_INTRINSIC(R, CLS, INFO)\
   BOOST_PP_CAT(BOOST_PP_OVERLOAD(__REGISTER_INTRINSIC, _UNWRAP_SEQ INFO) _EXPAND_ARGS(CLS, "asm2wasm", INFO), BOOST_PP_EMPTY())
...
class runtime_api_x : public context_aware_api {
   public:
      runtime_api_x()
      :context_aware_api(true){}
      double f64_rem(double a, double b) {
...
REGISTER_INJECTED_INTRINSICS(softfloat_api,
      ...
      (_eosio_i32_load,       int(int, int)         )
      (_eosio_i32_load8_s,    int(int, int)         )
      (_eosio_i32_load8_u,    int(int, int)         )
      (_eosio_i32_load16_s,   int(int, int)         )
      (_eosio_i32_load16_u,   int(int, int)         )

      (_eosio_i64_load,       int64_t(int, int)     )
      (_eosio_i64_load8_s,    int64_t(int, int)     )
      (_eosio_i64_load8_u,    int64_t(int, int)     )
      (_eosio_i64_load16_s,   int64_t(int, int)     )
      (_eosio_i64_load16_u,   int64_t(int, int)     )
      (_eosio_i64_load32_s,   int64_t(int, int)     )
      (_eosio_i64_load32_u,   int64_t(int, int)     )
