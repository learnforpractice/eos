...
#undef INSERT_INJECTED
   ...
   constexpr const char* inject_which_op( uint16_t opcode ) {
      switch ( opcode ) {
         ...
         default:
            EOS_THROW( wasm_execution_error, "Error, unknown opcode in injection ${op}", ("op", opcode));
      }
   }
   ...
   class wasm_binary_injection {
      ...
      public:
         ...
         void inject() {
            ...
            for (auto& itr: injector_utils::registered_injected) {
               ilog("+++++++++registered_injected ${n1} ${n2}", ("n1", itr.first)("n2", itr.second));
            }

            for (auto& itr: injector_utils::injected_index_mapping) {
               ilog("+++++++++injected_index_mapping ${n1} ${n2}", ("n1", itr.first)("n2", itr.second));
            }
         }
      private:
         IR::Module* _module;
