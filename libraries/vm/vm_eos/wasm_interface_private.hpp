...
namespace eosio { namespace chain {
   ...
   struct wasm_interface_impl {
      ...
      wasm_interface_impl(wasm_interface::vm_type vm, bool eosvmoc_tierup, const boost::filesystem::path data_dir, const eosvmoc::config& eosvmoc_config) : wasm_runtime_time(vm) {
         ...
         if(vm == wasm_interface::vm_type::eos_vm_oc) {
            chainbase::database* d = static_cast<chainbase::database *>(get_chain_api()->chain_get_database());
...
//            codeobject = &db.get<code_object,by_code_hash>(boost::make_tuple(code_hash, vm_type, vm_version));
            get_chain_api()->get_code_by_code_hash(code_hash, vm_type, vm_version, &contract_code, &contract_size, &first_block_used);
