...
u32 db_store_i64(u64 scope, u64 table, u64 payer, u64 id,  u32 data_offset, u32 len) {
...
void db_idx64_update(u32 iterator, u64 payer, u32 secondary_offset) {
   const uint64_t* secondary = (uint64_t *)offset_to_ptr(secondary_offset, sizeof(uint64_t));
   get_vm_api()->db_idx64_update(iterator, payer, secondary);
}

void db_idx64_remove(u32 iterator) {
   get_vm_api()->db_idx64_remove(iterator);
}

u32 db_idx64_next(u32 iterator, u32 primary_offset) {
   uint64_t *primary = (uint64_t *)offset_to_ptr(primary_offset, sizeof(uint64_t));
   return get_vm_api()->db_idx64_next(iterator, primary);
}

u32 db_idx64_previous(u32 iterator, u32 primary_offset) {
   uint64_t *primary = (uint64_t *)offset_to_ptr(primary_offset, sizeof(uint64_t));
   return get_vm_api()->db_idx64_previous(iterator, primary);
}

u32 db_idx64_find_primary(u64 code, u64 scope, u64 table, u32 secondary_offset, u64 primary) {
...
u32 db_idx64_find_secondary(u64 code, u64 scope, u64 table, u32 secondary_offset, u32 primary_offset) {
   const uint64_t* secondary = (uint64_t *)offset_to_ptr(secondary_offset, sizeof(uint64_t));
   uint64_t* primary = (uint64_t *)offset_to_ptr(primary_offset, sizeof(uint64_t));
   return get_vm_api()->db_idx64_find_secondary(code, scope, table, secondary, primary);
}

u32 db_idx64_lowerbound(u64 code, u64 scope, u64 table, u32 secondary_offset, u32 primary_offset) {
   uint64_t* secondary = (uint64_t *)offset_to_ptr(secondary_offset, sizeof(uint64_t));
   uint64_t* primary = (uint64_t *)offset_to_ptr(primary_offset, sizeof(uint64_t));
   return get_vm_api()->db_idx64_lowerbound(code, scope, table, secondary, primary);
}

u32 db_idx64_upperbound(u64 code, u64 scope, u64 table, u32 secondary_offset, u32 primary_offset) {
   uint64_t* secondary = (uint64_t *)offset_to_ptr(secondary_offset, sizeof(uint64_t));
   uint64_t* primary = (uint64_t *)offset_to_ptr(primary_offset, sizeof(uint64_t));
   return get_vm_api()->db_idx64_upperbound(code, scope, table, secondary, primary);
}

u32 db_idx64_end(u64 code, u64 scope, u64 table) {
   return get_vm_api()->db_idx64_end(code, scope, table);
}

u32 db_idx128_store(u64 scope, u64 table, u64 payer, u64 id, u32 secondary_offset) {
...
void db_idx128_update(u32 iterator, u64 payer, u32 secondary_offset) {
   const uint128_t* secondary = (uint128_t *)offset_to_ptr(secondary_offset, sizeof(uint128_t));
   get_vm_api()->db_idx128_update(iterator, payer, secondary);
}

void db_idx128_remove(u32 iterator) {
   get_vm_api()->db_idx128_remove(iterator);
}

u32 db_idx128_next(u32 iterator, u32 primary_offset) {
   uint64_t* primary = (uint64_t *)offset_to_ptr(primary_offset, sizeof(uint64_t));
   return get_vm_api()->db_idx128_next(iterator, primary);
}

u32 db_idx128_previous(u32 iterator, u32 primary_offset) {
   uint64_t* primary = (uint64_t *)offset_to_ptr(primary_offset, sizeof(uint64_t));
   return get_vm_api()->db_idx128_previous(iterator, primary) ;
}

u32 db_idx128_find_primary(u64 code, u64 scope, u64 table, u32 secondary_offset, u64 primary) {
...
u32 db_idx128_find_secondary(u64 code, u64 scope, u64 table, u32 secondary_offset, u32 primary_offset) {
   uint64_t* primary = (uint64_t *)offset_to_ptr(primary_offset, sizeof(uint64_t));
   uint128_t* secondary = (uint128_t *)offset_to_ptr(secondary_offset, sizeof(uint64_t));
   return get_vm_api()->db_idx128_find_secondary(code, scope, table, secondary, primary);
}

u32 db_idx128_lowerbound(u64 code, u64 scope, u64 table, u32 secondary_offset, u32 primary_offset) {
   uint64_t* primary = (uint64_t *)offset_to_ptr(primary_offset, sizeof(uint64_t));
   uint128_t* secondary = (uint128_t *)offset_to_ptr(secondary_offset, sizeof(uint64_t));
   return get_vm_api()->db_idx128_lowerbound(code, scope, table, secondary, primary);
}

u32 db_idx128_upperbound(u64 code, u64 scope, u64 table, u32 secondary_offset, u32 primary_offset) {
   uint64_t* primary = (uint64_t *)offset_to_ptr(primary_offset, sizeof(uint64_t));
   uint128_t* secondary = (uint128_t *)offset_to_ptr(secondary_offset, sizeof(uint64_t));
   return get_vm_api()->db_idx128_upperbound(code, scope, table, secondary, primary);
}

u32 db_idx128_end(u64 code, u64 scope, u64 table) {
   return get_vm_api()->db_idx128_end(code, scope, table);
}

u32 db_idx256_store(u64 scope, u64 table, u64 payer, u64 id, u32 data_offset, u32 data_len ) {
...
void db_idx256_update(u32 iterator, u64 payer, u32 data_offset, u32 data_len) {
   uint128_t* data = (uint128_t *)offset_to_ptr(data_offset, sizeof(uint128_t));
   get_vm_api()->db_idx256_update(iterator, payer, data, data_len);
}

void db_idx256_remove(u32 iterator) {
   get_vm_api()->db_idx256_remove(iterator);
}

u32 db_idx256_next(u32 iterator, u32 primary_offset) {
   uint64_t* primary = (uint64_t *)offset_to_ptr(primary_offset, sizeof(uint64_t));
   return get_vm_api()->db_idx256_next(iterator, primary);
}

u32 db_idx256_previous(u32 iterator, u32 primary_offset) {
   uint64_t* primary = (uint64_t *)offset_to_ptr(primary_offset, sizeof(uint64_t));
   return get_vm_api()->db_idx256_previous(iterator, primary);
}

u32 db_idx256_find_primary(u64 code, u64 scope, u64 table, u32 data_offset, u32 data_len, u64 primary) {
   uint128_t* data = (uint128_t *)offset_to_ptr(data_offset, sizeof(uint128_t));
   return get_vm_api()->db_idx256_find_primary(code, scope, table, data, data_len, primary);
}

u32 db_idx256_find_secondary(u64 code, u64 scope, u64 table, u32 data_offset, u32 data_len, u32 primary_offset) {
   uint64_t* primary = (uint64_t *)offset_to_ptr(primary_offset, sizeof(uint64_t));
...
u32 db_idx256_lowerbound(u64 code, u64 scope, u64 table, u32 data_offset, u32 data_len, u32 primary_offset) {
   uint64_t* primary = (uint64_t *)offset_to_ptr(primary_offset, sizeof(uint64_t));
   uint128_t* data = (uint128_t *)offset_to_ptr(data_offset, sizeof(uint128_t));
   return get_vm_api()->db_idx256_lowerbound(code, scope, table, data, data_len, primary);
}

u32 db_idx256_upperbound(u64 code, u64 scope, u64 table, u32 data_offset, u32 data_len, u32 primary_offset) {
   uint64_t* primary = (uint64_t *)offset_to_ptr(primary_offset, sizeof(uint64_t));
   uint128_t* data = (uint128_t *)offset_to_ptr(data_offset, sizeof(uint128_t));
   return get_vm_api()->db_idx256_upperbound(code, scope, table, data, data_len, primary);
}

u32 db_idx256_end(u64 code, u64 scope, u64 table) {
   return get_vm_api()->db_idx256_end(code, scope, table);
}

u32 db_idx_double_store(u64 scope, u64 table, u64 payer, u64 id, u32 secondary_offset) {
...
void db_idx_double_update(u32 iterator, u64 payer, u32 secondary_offset) {
   const double* secondary = (double*)offset_to_ptr(secondary_offset, sizeof(double));
   get_vm_api()->db_idx_double_update(iterator, payer, (const double*)secondary);
}

void db_idx_double_remove(u32 iterator) {
   get_vm_api()->db_idx_double_remove(iterator);
}

u32 db_idx_double_next(u32 iterator, u32 primary_offset) {
   uint64_t* primary = (uint64_t*)offset_to_ptr(primary_offset, sizeof(uint64_t));
   return get_vm_api()->db_idx_double_next(iterator, primary);
}

u32 db_idx_double_previous(u32 iterator, u32 primary_offset) {
   uint64_t* primary = (uint64_t*)offset_to_ptr(primary_offset, sizeof(uint64_t));
   return get_vm_api()->db_idx_double_previous(iterator, primary);
}

u32 db_idx_double_find_primary(u64 code, u64 scope, u64 table, u32 secondary_offset, u64 primary) {
...
u32 db_idx_double_find_secondary(u64 code, u64 scope, u64 table, u32 secondary_offset, u32 primary_offset) {
   uint64_t* primary = (uint64_t*)offset_to_ptr(primary_offset, sizeof(uint64_t));
   double* secondary = (double*)offset_to_ptr(secondary_offset, sizeof(double));
   return get_vm_api()->db_idx_double_find_secondary(code, scope, table, (const double*)secondary, primary);
}

u32 db_idx_double_lowerbound(u64 code, u64 scope, u64 table, u32 secondary_offset, u32 primary_offset) {
   uint64_t* primary = (uint64_t*)offset_to_ptr(primary_offset, sizeof(uint64_t));
   double* secondary = (double*)offset_to_ptr(secondary_offset, sizeof(double));
   return get_vm_api()->db_idx_double_lowerbound(code, scope, table, (double*)secondary, primary);
}

u32 db_idx_double_upperbound(u64 code, u64 scope, u64 table, u32 secondary_offset, u32 primary_offset) {
   uint64_t* primary = (uint64_t*)offset_to_ptr(primary_offset, sizeof(uint64_t));
   double* secondary = (double*)offset_to_ptr(secondary_offset, sizeof(double));
   return get_vm_api()->db_idx_double_upperbound(code, scope, table, (double*)secondary, primary);
}

u32 db_idx_double_end(u64 code, u64 scope, u64 table) {
   return get_vm_api()->db_idx_double_end(code, scope, table);
}

u32 db_idx_long_double_store(u64 scope, u64 table, u64 payer, u64 id, u32 secondary_offset) {
...
void db_idx_long_double_update(u32 iterator, u64 payer, u32 secondary_offset) {
   long double *secondary = (long double *)offset_to_ptr(secondary_offset, sizeof(long double));
   get_vm_api()->db_idx_long_double_update(iterator, payer, (const long double*)secondary);
}

void db_idx_long_double_remove(u32 iterator) {
   get_vm_api()->db_idx_long_double_remove(iterator);
}

u32 db_idx_long_double_next(u32 iterator, u32 primary_offset) {
   uint64_t* primary = (uint64_t*)offset_to_ptr(primary_offset, sizeof(uint64_t));
   return get_vm_api()->db_idx_long_double_next(iterator, primary);
}

u32 db_idx_long_double_previous(u32 iterator, u32 primary_offset) {
   uint64_t* primary = (uint64_t*)offset_to_ptr(primary_offset, sizeof(uint64_t));
   return get_vm_api()->db_idx_long_double_previous(iterator, primary);
}

u32 db_idx_long_double_find_primary(u64 code, u64 scope, u64 table, u32 secondary_offset, u64 primary) {
   long double* secondary = (long double*)offset_to_ptr(secondary_offset, sizeof(long double));
   return get_vm_api()->db_idx_long_double_find_primary(code, scope, table, (long double*)secondary, primary);
}

u32 db_idx_long_double_find_secondary(u64 code, u64 scope, u64 table, u32 secondary_offset, u32 primary_offset) {
   uint64_t* primary = (uint64_t*)offset_to_ptr(primary_offset, sizeof(uint64_t));
...
u32 db_idx_long_double_lowerbound(u64 code, u64 scope, u64 table, u32 secondary_offset, u32 primary_offset) {
   uint64_t* primary = (uint64_t*)offset_to_ptr(primary_offset, sizeof(uint64_t));
   const long double* secondary = (long double*)offset_to_ptr(secondary_offset, sizeof(long double));
   return get_vm_api()->db_idx_long_double_lowerbound(code, scope, table, (long double*)secondary, primary);
}

u32 db_idx_long_double_upperbound(u64 code, u64 scope, u64 table, u32 secondary_offset, u32 primary_offset) {
   uint64_t* primary = (uint64_t*)offset_to_ptr(primary_offset, sizeof(uint64_t));
   const long double* secondary = (long double*)offset_to_ptr(secondary_offset, sizeof(long double));
   return get_vm_api()->db_idx_long_double_upperbound(code, scope, table, (long double*)secondary, primary);
}

u32 db_idx_long_double_end(u64 code, u64 scope, u64 table) {
   return get_vm_api()->db_idx_long_double_end(code, scope, table);
}

// u32 db_get_table_row_count(u64 code, u64 scope, u64 table) {
//    return get_vm_api()->db_get_table_row_count(code, scope, table);
// }

// u32 db_find_i256( u64 code, u64 scope, u64 table, u32 id_offset, u32 id_size ) {
//    void *id = (void *)offset_to_ptr(id_offset, id_size);
//    return get_vm_api()->db_find_i256(code, scope, table, id, id_size);
// }

// u32 db_store_i256( u64 scope, u64 table, u64 payer, u32 id_offset, u32 id_size, u32 buffer_offset, u32 buffer_size ) {
//    void *id = (void *)offset_to_ptr(id_offset, id_size);
