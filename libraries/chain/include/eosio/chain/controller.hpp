...
FC_REFLECT(eosio::chain::controller::config, 
    ...
    (sig_cpu_bill_pct)
    (thread_pool_size)
    (max_retained_block_files)
    (blocks_log_stride)
    (backing_store)
    (persistent_storage_num_threads)
    (persistent_storage_max_num_files)
    (persistent_storage_write_buffer_size)
    (persistent_storage_bytes_per_sync)
    (persistent_storage_mbytes_batch)
...
FC_REFLECT(eosio::chain::block_log_config,
                        (log_dir)
                        (retained_dir)
                        (archive_dir)
                        (stride)
