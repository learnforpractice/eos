...
void vm_api_proxy::send_inline( const char *data, size_t size ) {
    legacy_span<const char> _data((void *)data, size);
    return _interface->send_inline(std::move(_data));
}

void vm_api_proxy::send_context_free_inline( const char *data, size_t size ) {
    legacy_span<const char> _data((void *)data, size);
    return _interface->send_context_free_inline(std::move(_data));    
}

void vm_api_proxy::send_deferred( const __uint128 *sender_id, uint64_t payer, const char *data, size_t size, uint32_t replace_existing) {
...
bool vm_api_proxy::cancel_deferred( const __uint128 *sender_id ) {
   legacy_ptr<const uint128_t> _sender_id((void *)sender_id);
