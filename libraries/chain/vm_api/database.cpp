...
int32_t vm_api_proxy::db_store_i64( uint64_t scope, uint64_t table, uint64_t payer, uint64_t id, const char *data, size_t size ) {
    legacy_span<const char> buffer((void *)data, size);
    return _interface->db_store_i64(scope, table, payer, id, std::move(buffer));
}

void vm_api_proxy::db_update_i64( int32_t itr, uint64_t payer, const char *data, size_t size) {
    legacy_span<const char> buffer((void *)data, size);
...
int32_t vm_api_proxy::db_get_i64( int32_t itr, char *data, size_t size ) {
...
/**
   ...
   */
int32_t vm_api_proxy::db_idx256_store( uint64_t scope, uint64_t table, uint64_t payer, uint64_t id, const __uint128 *data, size_t data_len ) {
...
void vm_api_proxy::db_idx256_update( int32_t iterator, uint64_t payer, const __uint128 *data, size_t data_len ) {
    legacy_span<const uint128_t> _data((void *)data, data_len);
...
int32_t vm_api_proxy::db_idx256_find_secondary( uint64_t code, uint64_t scope, uint64_t table, const __uint128 *data, size_t data_len, uint64_t *primary ) {
    legacy_span<const uint128_t> _data((void *)data, data_len);
    legacy_ptr<uint64_t> _primary(primary);
    return _interface->db_idx256_find_secondary(code, scope, table, std::move(_data), std::move(_primary));
}

int32_t vm_api_proxy::db_idx256_find_primary( uint64_t code, uint64_t scope, uint64_t table, __uint128 *data, size_t data_len, uint64_t primary ) {
...
int32_t vm_api_proxy::db_idx256_lowerbound( uint64_t code, uint64_t scope, uint64_t table, __uint128 *data, size_t data_len, uint64_t *primary ) {
    legacy_span<uint128_t> _data(data, data_len);
    legacy_ptr<uint64_t> _primary(primary);
    return _interface->db_idx256_lowerbound(code, scope, table, std::move(_data), std::move(_primary));
}

int32_t vm_api_proxy::db_idx256_upperbound( uint64_t code, uint64_t scope, uint64_t table, __uint128 *data, size_t data_len, uint64_t *primary ) {
    legacy_span<uint128_t> _data(data, data_len);
    legacy_ptr<uint64_t> _primary(primary);
...
/**
   ...
   */
int32_t vm_api_proxy::db_idx_double_store( uint64_t scope, uint64_t table, uint64_t payer, uint64_t id, const float64_t *secondary ) {
...
void vm_api_proxy::db_idx_double_update( int32_t iterator, uint64_t payer, const float64_t *secondary ) {
    legacy_ptr<const float64_t> _secondary((void *)secondary);
...
int32_t vm_api_proxy::db_idx_double_find_secondary( uint64_t code, uint64_t scope, uint64_t table, const float64_t *secondary, uint64_t *primary ) {
    legacy_ptr<const float64_t> _secondary((void *)secondary);
    legacy_ptr<uint64_t> _primary(primary);
    return _interface->db_idx_double_find_secondary(code, scope, table, std::move(_secondary), std::move(_primary));
}

int32_t vm_api_proxy::db_idx_double_find_primary( uint64_t code, uint64_t scope, uint64_t table, float64_t *secondary, uint64_t primary ) {
...
int32_t vm_api_proxy::db_idx_double_lowerbound( uint64_t code, uint64_t scope, uint64_t table, float64_t *secondary, uint64_t *primary ) {
    legacy_ptr<float64_t> _secondary(secondary);
    legacy_ptr<uint64_t> _primary(primary);
    return _interface->db_idx_double_lowerbound(code, scope, table, std::move(_secondary), std::move(_primary));
}

int32_t vm_api_proxy::db_idx_double_upperbound( uint64_t code, uint64_t scope, uint64_t table, float64_t *secondary, uint64_t *primary ) {
    legacy_ptr<float64_t> _secondary(secondary);
    legacy_ptr<uint64_t> _primary(primary);
...
/**
   ...
   */
int32_t vm_api_proxy::db_idx_long_double_store( uint64_t scope, uint64_t table, uint64_t payer, uint64_t id, const float128_t *secondary ) {
...
void vm_api_proxy::db_idx_long_double_update( int32_t iterator, uint64_t payer, const float128_t *secondary ) {
    legacy_ptr<const float128_t> _secondary((void *)secondary);
...
int32_t vm_api_proxy::db_idx_long_double_find_secondary( uint64_t code, uint64_t scope, uint64_t table, const float128_t *secondary, uint64_t *primary ) {
    legacy_ptr<const float128_t> _secondary((void *)secondary);
    legacy_ptr<uint64_t> _primary(primary);
    return _interface->db_idx_long_double_find_secondary(code, scope, table, std::move(_secondary), std::move(_primary));
}

int32_t vm_api_proxy::db_idx_long_double_find_primary( uint64_t code, uint64_t scope, uint64_t table, float128_t *secondary, uint64_t primary ) {
...
int32_t vm_api_proxy::db_idx_long_double_lowerbound( uint64_t code, uint64_t scope, uint64_t table, float128_t *secondary, uint64_t *primary ) {
    legacy_ptr<float128_t> _secondary(secondary);
    legacy_ptr<uint64_t> _primary(primary);
    return _interface->db_idx_long_double_lowerbound(code, scope, table, std::move(_secondary), std::move(_primary));
}

int32_t vm_api_proxy::db_idx_long_double_upperbound( uint64_t code, uint64_t scope, uint64_t table, float128_t *secondary, uint64_t *primary ) {
    legacy_ptr<float128_t> _secondary(secondary);
    legacy_ptr<uint64_t> _primary(primary);
