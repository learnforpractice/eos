...
string db_api::exec_action(uint64_t code, uint64_t action, const vector<char>& args) {
...
int db_api::db_previous_i256( int iterator, key256_t& primary ) {
...
int db_api::db_next_i256( int iterator, key256_t& primary ) {
   if( iterator < -1 ) return -1; // cannot increment past end iterator of table
