...
public:
   ...
   eosio_system_tester(Lambda setup) {
      ...
      create_accounts({ N(uuos.token), N(uuos.ram), N(uuos.ramfee), N(uuos.stake),
               N(uuos.bpay), N(uuos.vpay), N(uuos.saving), N(uuos.names) });

      produce_blocks( 100 );

      set_code( N(uuos.token), contracts::eosio_token_wasm() );
      set_abi( N(uuos.token), contracts::eosio_token_abi().data() );
      ...
      create_currency( N(uuos.token), config::system_account_name, core_from_string("10000000000.0000") );
   ...
   asset get_balance( const account_name& act ) {
      vector<char> data = get_row_by_account( N(uuos.token), act, N(accounts), name(symbol(CORE_SYMBOL).to_symbol_code().value) );
   ...
   fc::variant get_stats( const string& symbolname ) {
      ...
      vector<char> data = get_row_by_account( N(uuos.token), name(symbol_code), N(stat), name(symbol_code) );
   ...
   abi_serializer initialize_multisig() {
      abi_serializer msig_abi_ser;
      {
         ...
         set_code( N(uuos.msig), contracts::eosio_msig_wasm() );
         set_abi( N(uuos.msig), contracts::eosio_msig_abi().data() );
