...
class eosio_token_tester_v2 : public tester {
public:

   eosio_token_tester_v2() {
      produce_blocks( 2 );

      create_accounts( { N(alice), N(bob), N(carol), N(eosio.token) } );
      produce_blocks( 2 );

      set_code( N(eosio.token), contracts::eosio_token_v2_wasm() );
      set_abi( N(eosio.token), contracts::eosio_token_v2_abi().data() );
...
BOOST_AUTO_TEST_SUITE(eosio_token_v2_tests)

BOOST_FIXTURE_TEST_CASE( create_tests, eosio_token_tester_v2 ) try {
...
BOOST_FIXTURE_TEST_CASE( create_negative_max_supply, eosio_token_tester_v2 ) try {
...
BOOST_FIXTURE_TEST_CASE( symbol_already_exists, eosio_token_tester_v2 ) try {
...
BOOST_FIXTURE_TEST_CASE( create_max_supply, eosio_token_tester_v2 ) try {
...
BOOST_FIXTURE_TEST_CASE( create_max_decimals, eosio_token_tester_v2 ) try {
...
BOOST_FIXTURE_TEST_CASE( issue_tests, eosio_token_tester_v2 ) try {
...
BOOST_FIXTURE_TEST_CASE( transfer_tests, eosio_token_tester_v2 ) try {
