...
public:
   eosio_msig_tester() {
      create_accounts( { N(uuos.msig), N(uuos.stake), N(uuos.ram), N(uuos.ramfee), N(alice), N(bob), N(carol) } );
      produce_block();
      ...
      set_code( N(uuos.msig), contracts::msig_wasm() );
      set_abi( N(uuos.msig), contracts::msig_abi().data() );

      produce_blocks();
      const auto& accnt = control->db().get<account_object,by_name>( N(uuos.msig) );
      abi_def abi;
   ...
   transaction_trace_ptr create_account_with_resources( account_name a, account_name creator, asset ramfunds, bool multisig,
      ...
      trx.actions.emplace_back( get_action( N(uuos), N(buyram), vector<permission_level>{{creator,config::active_name}},
                                            mvo()
                                            ("payer", creator)
                                            ("receiver", a)
                                            ("quant", ramfunds) )
      ...
      trx.actions.emplace_back( get_action( N(uuos), N(delegatebw), vector<permission_level>{{creator,config::active_name}},
                                            mvo()
                                            ("from", creator)
                                            ("receiver", a)
   ...
   void issue( name to, const asset& amount, name manager = config::system_account_name ) {
      base_tester::push_action( N(uuos.token), N(issue), manager, mutable_variant_object()
                                ("to",      to )
                                ("quantity", amount )
                                ("memo", "")
                                );
   ...
   void transfer( name from, name to, const string& amount, name manager = config::system_account_name ) {
      base_tester::push_action( N(uuos.token), N(transfer), manager, mutable_variant_object()
                                ("from",    from)
                                ("to",      to )
   ...
   asset get_balance( const account_name& act ) {
      ...
      const auto* tbl = db.find<table_id_object, by_code_scope_table>(boost::make_tuple(N(uuos.token), act, N(accounts)));
      share_type result = 0;
   ...
   transaction_trace_ptr push_action( const account_name& signer, const action_name& name, const variant_object& data, bool auth = true ) {
      ...
      auto trace = base_tester::push_action( N(uuos.msig), name, accounts, data );
      produce_block();
...
BOOST_FIXTURE_TEST_CASE( update_system_contract_all_approve, eosio_msig_tester ) try {
   ...
   set_authority(
      ...
      authority( 1,
                 ...
                 vector<permission_level_weight>{{{N(uuos.prods), config::active_name}, 1}}
      ),
      config::owner_name,
   ...
   create_accounts( { N(uuos.token), N(uuos.rex) } );
   set_code( N(uuos.token), contracts::token_wasm() );
   set_abi( N(uuos.token), contracts::token_abi().data() );

   create_currency( N(uuos.token), config::system_account_name, core_sym::from_string("10000000000.0000") );
   ...
   BOOST_REQUIRE_EQUAL( core_sym::from_string("1000000000.0000"),
                        get_balance(config::system_account_name) + get_balance(N(uuos.ramfee)) + get_balance(N(uuos.stake)) + get_balance(N(uuos.ram)) );
   ...
   produce_blocks();
   create_account_with_resources( N(alice1111111), N(uuos), core_sym::from_string("1.0000"), false );
   create_account_with_resources( N(bob111111111), N(uuos), core_sym::from_string("0.4500"), false );
   create_account_with_resources( N(carol1111111), N(uuos), core_sym::from_string("1.0000"), false );
   ...
   vector<permission_level> action_perm = {{N(uuos), config::active_name}};
   ...
   BOOST_REQUIRE_EXCEPTION( create_account_with_resources( N(alice1111112), N(uuos), core_sym::from_string("1.0000"), false ),
...
BOOST_FIXTURE_TEST_CASE( approve_execute_old, eosio_msig_tester ) try {
   set_code( N(uuos.msig), contracts::util::msig_wasm_old() );
   set_abi( N(uuos.msig), contracts::util::msig_abi_old().data() );
   produce_blocks();
