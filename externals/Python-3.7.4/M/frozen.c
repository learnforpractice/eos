extern unsigned char M___future__[];
extern unsigned char M___main__[];
extern unsigned char M__bootlocale[];
extern unsigned char M__collections_abc[];
extern unsigned char M__compat_pickle[];
extern unsigned char M__compression[];
extern unsigned char M__frozen_importlib[];
extern unsigned char M__frozen_importlib_external[];
extern unsigned char M__py_abc[];
extern unsigned char M__sitebuiltins[];
extern unsigned char M__strptime[];
extern unsigned char M__threading_local[];
extern unsigned char M__weakrefset[];
extern unsigned char M_abc[];
extern unsigned char M_argparse[];
extern unsigned char M_ast[];
extern unsigned char M_base64[];
extern unsigned char M_bdb[];
extern unsigned char M_bisect[];
extern unsigned char M_bz2[];
extern unsigned char M_calendar[];
extern unsigned char M_cmd[];
extern unsigned char M_code[];
extern unsigned char M_codecs[];
extern unsigned char M_codeop[];
extern unsigned char M_collections[];
extern unsigned char M_collections__abc[];
extern unsigned char M_contextlib[];
extern unsigned char M_copy[];
extern unsigned char M_copyreg[];
extern unsigned char M_datetime[];
extern unsigned char M_difflib[];
extern unsigned char M_dis[];
extern unsigned char M_doctest[];
extern unsigned char M_email[];
extern unsigned char M_email___encoded_words[];
extern unsigned char M_email___header_value_parser[];
extern unsigned char M_email___parseaddr[];
extern unsigned char M_email___policybase[];
extern unsigned char M_email__base64mime[];
extern unsigned char M_email__charset[];
extern unsigned char M_email__contentmanager[];
extern unsigned char M_email__encoders[];
extern unsigned char M_email__errors[];
extern unsigned char M_email__feedparser[];
extern unsigned char M_email__generator[];
extern unsigned char M_email__header[];
extern unsigned char M_email__headerregistry[];
extern unsigned char M_email__iterators[];
extern unsigned char M_email__message[];
extern unsigned char M_email__parser[];
extern unsigned char M_email__policy[];
extern unsigned char M_email__quoprimime[];
extern unsigned char M_email__utils[];
extern unsigned char M_encodings[];
extern unsigned char M_encodings__aliases[];
extern unsigned char M_encodings__latin_1[];
extern unsigned char M_encodings__mbcs[];
extern unsigned char M_encodings__utf_8[];
extern unsigned char M_enum[];
extern unsigned char M_fnmatch[];
extern unsigned char M_functools[];
extern unsigned char M_genericpath[];
extern unsigned char M_getopt[];
extern unsigned char M_gettext[];
extern unsigned char M_glob[];
extern unsigned char M_gzip[];
extern unsigned char M_hashlib[];
extern unsigned char M_heapq[];
extern unsigned char M_html[];
extern unsigned char M_html__entities[];
extern unsigned char M_http[];
extern unsigned char M_http__client[];
extern unsigned char M_http__server[];
extern unsigned char M_importlib[];
extern unsigned char M_importlib___bootstrap[];
extern unsigned char M_importlib___bootstrap_external[];
extern unsigned char M_importlib__abc[];
extern unsigned char M_importlib__machinery[];
extern unsigned char M_importlib__util[];
extern unsigned char M_inspect[];
extern unsigned char M_io[];
extern unsigned char M_keyword[];
extern unsigned char M_linecache[];
extern unsigned char M_locale[];
extern unsigned char M_logging[];
extern unsigned char M_lzma[];
extern unsigned char M_mimetypes[];
extern unsigned char M_ntpath[];
extern unsigned char M_opcode[];
extern unsigned char M_operator[];
extern unsigned char M_optparse[];
extern unsigned char M_os[];
extern unsigned char M_pdb[];
extern unsigned char M_pickle[];
extern unsigned char M_pkgutil[];
extern unsigned char M_platform[];
extern unsigned char M_plistlib[];
extern unsigned char M_posixpath[];
extern unsigned char M_pprint[];
extern unsigned char M_py_compile[];
extern unsigned char M_pydoc[];
extern unsigned char M_pydoc_data[];
extern unsigned char M_pydoc_data__topics[];
extern unsigned char M_quopri[];
extern unsigned char M_random[];
extern unsigned char M_re[];
extern unsigned char M_reprlib[];
extern unsigned char M_rlcompleter[];
extern unsigned char M_runpy[];
extern unsigned char M_selectors[];
extern unsigned char M_shlex[];
extern unsigned char M_shutil[];
extern unsigned char M_signal[];
extern unsigned char M_site[];
extern unsigned char M_socket[];
extern unsigned char M_socketserver[];
extern unsigned char M_sre_compile[];
extern unsigned char M_sre_constants[];
extern unsigned char M_sre_parse[];
extern unsigned char M_ssl[];
extern unsigned char M_stat[];
extern unsigned char M_string[];
extern unsigned char M_struct[];
extern unsigned char M_subprocess[];
extern unsigned char M_tarfile[];
extern unsigned char M_tempfile[];
extern unsigned char M_textwrap[];
extern unsigned char M_threading[];
extern unsigned char M_token[];
extern unsigned char M_tokenize[];
extern unsigned char M_traceback[];
extern unsigned char M_tracemalloc[];
extern unsigned char M_tty[];
extern unsigned char M_types[];
extern unsigned char M_typing[];
extern unsigned char M_unittest[];
extern unsigned char M_unittest__case[];
extern unsigned char M_unittest__loader[];
extern unsigned char M_unittest__main[];
extern unsigned char M_unittest__result[];
extern unsigned char M_unittest__runner[];
extern unsigned char M_unittest__signals[];
extern unsigned char M_unittest__suite[];
extern unsigned char M_unittest__util[];
extern unsigned char M_urllib[];
extern unsigned char M_urllib__parse[];
extern unsigned char M_uu[];
extern unsigned char M_warnings[];
extern unsigned char M_weakref[];
extern unsigned char M_webbrowser[];
extern unsigned char M_xml[];
extern unsigned char M_xml__parsers[];
extern unsigned char M_xml__parsers__expat[];
extern unsigned char M_zipfile[];

#include "Python.h"

static struct _frozen _PyImport_FrozenModules[] = {
	{"__future__", M___future__, 4117},
	{"__main__", M___main__, 107},
	{"_bootlocale", M__bootlocale, 1234},
	{"_collections_abc", M__collections_abc, 28927},
	{"_compat_pickle", M__compat_pickle, 5793},
	{"_compression", M__compression, 4109},
	{"_frozen_importlib", M__frozen_importlib, 29163},
	{"_frozen_importlib_external", M__frozen_importlib_external, 41803},
	{"_py_abc", M__py_abc, 4639},
	{"_sitebuiltins", M__sitebuiltins, 3450},
	{"_strptime", M__strptime, 16089},
	{"_threading_local", M__threading_local, 6397},
	{"_weakrefset", M__weakrefset, 7447},
	{"abc", M_abc, 6436},
	{"argparse", M_argparse, 61898},
	{"ast", M_ast, 12065},
	{"base64", M_base64, 16973},
	{"bdb", M_bdb, 24632},
	{"bisect", M_bisect, 2683},
	{"bz2", M_bz2, 11166},
	{"calendar", M_calendar, 27409},
	{"cmd", M_cmd, 12575},
	{"code", M_code, 9843},
	{"codecs", M_codecs, 34060},
	{"codeop", M_codeop, 6275},
	{"collections", M_collections, -47061},
	{"collections.abc", M_collections__abc, 190},
	{"contextlib", M_contextlib, 20443},
	{"copy", M_copy, 7086},
	{"copyreg", M_copyreg, 4229},
	{"datetime", M_datetime, 57234},
	{"difflib", M_difflib, 59423},
	{"dis", M_dis, 15190},
	{"doctest", M_doctest, 75419},
	{"email", M_email, -1676},
	{"email._encoded_words", M_email___encoded_words, 5593},
	{"email._header_value_parser", M_email___header_value_parser, 76128},
	{"email._parseaddr", M_email___parseaddr, 12333},
	{"email._policybase", M_email___policybase, 14835},
	{"email.base64mime", M_email__base64mime, 3220},
	{"email.charset", M_email__charset, 11514},
	{"email.contentmanager", M_email__contentmanager, 7280},
	{"email.encoders", M_email__encoders, 1649},
	{"email.errors", M_email__errors, 6176},
	{"email.feedparser", M_email__feedparser, 10614},
	{"email.generator", M_email__generator, 12485},
	{"email.header", M_email__header, 16371},
	{"email.headerregistry", M_email__headerregistry, 21282},
	{"email.iterators", M_email__iterators, 1917},
	{"email.message", M_email__message, 37925},
	{"email.parser", M_email__parser, 5732},
	{"email.policy", M_email__policy, 9640},
	{"email.quoprimime", M_email__quoprimime, 7649},
	{"email.utils", M_email__utils, 9452},
	{"encodings", M_encodings, -3932},
	{"encodings.aliases", M_encodings__aliases, 6281},
	{"encodings.latin_1", M_encodings__latin_1, 1881},
	{"encodings.mbcs", M_encodings__mbcs, 1680},
	{"encodings.utf_8", M_encodings__utf_8, 1599},
	{"enum", M_enum, 24256},
	{"fnmatch", M_fnmatch, 3322},
	{"functools", M_functools, 24200},
	{"genericpath", M_genericpath, 3733},
	{"getopt", M_getopt, 6224},
	{"gettext", M_gettext, 14153},
	{"glob", M_glob, 4244},
	{"gzip", M_gzip, 17170},
	{"hashlib", M_hashlib, 6576},
	{"heapq", M_heapq, 14347},
	{"html", M_html, -3382},
	{"html.entities", M_html__entities, 50454},
	{"http", M_http, -5907},
	{"http.client", M_http__client, 34301},
	{"http.server", M_http__server, 33352},
	{"importlib", M_importlib, -3717},
	{"importlib._bootstrap", M_importlib___bootstrap, 29163},
	{"importlib._bootstrap_external", M_importlib___bootstrap_external, 41803},
	{"importlib.abc", M_importlib__abc, 13472},
	{"importlib.machinery", M_importlib__machinery, 957},
	{"importlib.util", M_importlib__util, 9341},
	{"inspect", M_inspect, 80022},
	{"io", M_io, 3394},
	{"keyword", M_keyword, 1794},
	{"linecache", M_linecache, 3774},
	{"locale", M_locale, 34540},
	{"logging", M_logging, -62387},
	{"lzma", M_lzma, 11924},
	{"mimetypes", M_mimetypes, 15463},
	{"ntpath", M_ntpath, 12989},
	{"opcode", M_opcode, 5363},
	{"operator", M_operator, 13885},
	{"optparse", M_optparse, 47878},
	{"os", M_os, 29678},
	{"pdb", M_pdb, 46913},
	{"pickle", M_pickle, 43026},
	{"pkgutil", M_pkgutil, 16345},
	{"platform", M_platform, 28149},
	{"plistlib", M_plistlib, 25080},
	{"posixpath", M_posixpath, 10414},
	{"pprint", M_pprint, 15818},
	{"py_compile", M_py_compile, 7176},
	{"pydoc", M_pydoc, 84415},
	{"pydoc_data", M_pydoc_data, -131},
	{"pydoc_data.topics", M_pydoc_data__topics, 411717},
	{"quopri", M_quopri, 5756},
	{"random", M_random, 19393},
	{"re", M_re, 13789},
	{"reprlib", M_reprlib, 5335},
	{"rlcompleter", M_rlcompleter, 5732},
	{"runpy", M_runpy, 7930},
	{"selectors", M_selectors, 16933},
	{"shlex", M_shlex, 6975},
	{"shutil", M_shutil, 30965},
	{"signal", M_signal, 2497},
	{"site", M_site, 16535},
	{"socket", M_socket, 22006},
	{"socketserver", M_socketserver, 24184},
	{"sre_compile", M_sre_compile, 15188},
	{"sre_constants", M_sre_constants, 6276},
	{"sre_parse", M_sre_parse, 21342},
	{"ssl", M_ssl, 39375},
	{"stat", M_stat, 3858},
	{"string", M_string, 7820},
	{"struct", M_struct, 319},
	{"subprocess", M_subprocess, 38773},
	{"tarfile", M_tarfile, 61826},
	{"tempfile", M_tempfile, 22130},
	{"textwrap", M_textwrap, 13597},
	{"threading", M_threading, 37887},
	{"token", M_token, 3584},
	{"tokenize", M_tokenize, 17816},
	{"traceback", M_traceback, 19608},
	{"tracemalloc", M_tracemalloc, 17261},
	{"tty", M_tty, 1079},
	{"types", M_types, 8961},
	{"typing", M_typing, 49965},
	{"unittest", M_unittest, -2996},
	{"unittest.case", M_unittest__case, 48343},
	{"unittest.loader", M_unittest__loader, 14261},
	{"unittest.main", M_unittest__main, 7423},
	{"unittest.result", M_unittest__result, 7237},
	{"unittest.runner", M_unittest__runner, 6980},
	{"unittest.signals", M_unittest__signals, 2179},
	{"unittest.suite", M_unittest__suite, 9188},
	{"unittest.util", M_unittest__util, 4506},
	{"urllib", M_urllib, -127},
	{"urllib.parse", M_urllib__parse, 30818},
	{"uu", M_uu, 3557},
	{"warnings", M_warnings, 13925},
	{"weakref", M_weakref, 19559},
	{"webbrowser", M_webbrowser, 16359},
	{"xml", M_xml, -691},
	{"xml.parsers", M_xml__parsers, -304},
	{"xml.parsers.expat", M_xml__parsers__expat, 333},
	{"zipfile", M_zipfile, 49872},

    {0, 0, 0} /* sentinel */
};

#include "Python.h"
#include "internal/pystate.h"
#include <locale.h>

#ifdef MS_WINDOWS
extern void PyWinFreeze_ExeInit(void);
extern void PyWinFreeze_ExeTerm(void);
extern int PyInitFrozenExtensions(void);
#endif

/* Main program */

int Py_InitFrozenMain(int argc, char **argv)
{
	PyImport_FrozenModules = _PyImport_FrozenModules;

    _PyInitError err = _PyRuntime_Initialize();
    if (_Py_INIT_FAILED(err)) {
        fprintf(stderr, "Fatal Python error: %s\n", err.msg);
        fflush(stderr);
        exit(1);
    }

    const char *p;
    int i, n, sts = 1;
    int inspect = 0;
    int unbuffered = 0;
    char *oldloc = NULL;
    wchar_t **argv_copy = NULL;
    /* We need a second copies, as Python might modify the first one. */
    wchar_t **argv_copy2 = NULL;

    if (argc > 0) {
        argv_copy = PyMem_RawMalloc(sizeof(wchar_t*) * argc);
        argv_copy2 = PyMem_RawMalloc(sizeof(wchar_t*) * argc);
        if (!argv_copy || !argv_copy2) {
            fprintf(stderr, "out of memory\n");
            goto error;
        }
    }

    Py_FrozenFlag = 1; /* Suppress errors from getpath.c */

    if ((p = Py_GETENV("PYTHONINSPECT")) && *p != '\0')
        inspect = 1;
    if ((p = Py_GETENV("PYTHONUNBUFFERED")) && *p != '\0')
        unbuffered = 1;

    if (unbuffered) {
        setbuf(stdin, (char *)NULL);
        setbuf(stdout, (char *)NULL);
        setbuf(stderr, (char *)NULL);
    }

    oldloc = _PyMem_RawStrdup(setlocale(LC_ALL, NULL));
    if (!oldloc) {
        fprintf(stderr, "out of memory\n");
        goto error;
    }

    setlocale(LC_ALL, "");
    for (i = 0; i < argc; i++) {
        argv_copy[i] = Py_DecodeLocale(argv[i], NULL);
        argv_copy2[i] = argv_copy[i];
        if (!argv_copy[i]) {
            fprintf(stderr, "Unable to decode the command line argument #%i\n",
                            i + 1);
            argc = i;
            goto error;
        }
    }
    setlocale(LC_ALL, oldloc);
    PyMem_RawFree(oldloc);
    oldloc = NULL;

#ifdef MS_WINDOWS
    PyInitFrozenExtensions();
#endif /* MS_WINDOWS */
    if (argc >= 1)
        Py_SetProgramName(argv_copy[0]);
    Py_Initialize();
#ifdef MS_WINDOWS
    PyWinFreeze_ExeInit();
#endif

    if (Py_VerboseFlag)
        fprintf(stderr, "Python %s\n%s\n",
            Py_GetVersion(), Py_GetCopyright());

    PySys_SetArgv(argc, argv_copy);

    n = PyImport_ImportFrozenModule("__main__");
    if (n == 0)
        Py_FatalError("__main__ not frozen");
    if (n < 0) {
        PyErr_Print();
        sts = 1;
    }
    else
        sts = 0;

    if (inspect && isatty((int)fileno(stdin)))
        sts = PyRun_AnyFile(stdin, "<stdin>") != 0;

    return sts;

#ifdef MS_WINDOWS
    PyWinFreeze_ExeTerm();
#endif
    if (Py_FinalizeEx() < 0) {
        sts = 120;
    }

error:
    PyMem_RawFree(argv_copy);
    if (argv_copy2) {
        for (i = 0; i < argc; i++)
            PyMem_RawFree(argv_copy2[i]);
        PyMem_RawFree(argv_copy2);
    }
    PyMem_RawFree(oldloc);
    return sts;
}


int
main_(int argc, char **argv)
{
        extern int Py_FrozenMain(int, char **);

        PyImport_FrozenModules = _PyImport_FrozenModules;
        return Py_FrozenMain(argc, argv);
}

